// lobby.js - 轉蛋大廳、星塵商店、圖鑑與單字本 UI (強制小寫路徑版)
...
function createLobbyUI() {
    ...
    if(startScreen) {
        ...
        openBtn.onclick = openLobby; startScreen.appendChild(openBtn);
    }
}
...
function buildReportUI() {
    let rc = document.getElementById('report-container');
    if(!rc) return;
...
function buildLessonUI() {
    let lcc = document.getElementById('lesson-checkbox-container'); if(!lcc) return;
    lcc.innerHTML = "";
...
function openLobby() { 
    ...
    buildLessonUI(); 
    buildReportUI(); 
    document.getElementById('gacha-lobby').style.display = 'flex'; 
...
function closeLobby() { 
    document.getElementById('gacha-lobby').style.display = 'none'; 
    checkUnlocks(); 
    changeHeroSelection(0); 
    if(typeof lobbyBgm !== 'undefined') { lobbyBgm.pause(); lobbyBgm.currentTime = 0; }
...
function switchTab(tabId) {
    ...
    if (tabId === 'shop') {
        ...
        if(window.shopBgm) window.shopBgm.pause();
        if(window.gachaBgm) window.gachaBgm.pause();
        if(document.getElementById('gacha-lobby').style.display !== 'none' && typeof lobbyBgm !== 'undefined') {
            lobbyBgm.play().catch(e=>null);
        }
    }

    document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
    document.querySelectorAll('.lobby-content').forEach(c => c.classList.remove('active'));
    let targetBtn = document.querySelector(`.tab-btn[onclick="switchTab('${tabId}')"]`); if(targetBtn) targetBtn.classList.add('active');
    let tab = document.getElementById('tab-' + tabId); if(tab) tab.classList.add('active');
}

function updateLobbyUI() {
    if(typeof calcBuffs === 'function') calcBuffs();
    document.getElementById('lobby-coin-val').innerText = window.gachaData.coins;
    document.getElementById('lobby-dust-val').innerText = window.gachaData.dust;
    let sdd = document.getElementById('shop-dust-display');
    if(sdd) sdd.innerText = window.gachaData.dust;
    
    document.getElementById('buff-panel').innerHTML = `✨ 目前全體加成：血量 +${window.hpBuffAmt}💖 | 爆擊率 ${window.critRate}%💥 | 提示 +${window.hintBuffAmt}🪄 | 額外代幣 +${window.extraCoinsBonus}🪙`;
    ...
    let grid = document.getElementById('main-sticker-grid'); 
    if(grid) {
        grid.innerHTML = "";
        stickerDB.forEach(s => {
            if(!enabledStickers.includes(s.id)) return;
                ...
                if(isOwned) {
                    ...
                    if(modal) {
                        ...
                        if(dImg) { 
                            ...
                            dImg.classList.remove('ssr-glow', 'sr-glow', 'r-glow');
                            if(s.rarity === 'SSR') dImg.classList.add('ssr-glow'); else if(s.rarity === 'SR') dImg.classList.add('sr-glow'); else if(s.rarity === 'R') dImg.classList.add('r-glow');
                        }
                        let matchedHero = (s.id < 10) ? heroes[s.id] : null;
                        ...
                        let dTitle = document.getElementById('detail-title'), dDesc = document.getElementById('detail-desc');
                        let starText = stars > 0 ? `\n\n⭐ 目前星級：${stars} / 5` : "\n\n⭐ 目前星級：0 / 5";
                        if(stars >= 5) starText += " (已解鎖專屬短片！)";

                        if(matchedHero) {
                            let is5Star = stars >= 5; let s4Text = is5Star ? `4. ${matchedHero.skills[3]} (🌟五星解鎖/限一次)` : `4. 🔒 (滿五星解鎖隱藏招式)`;
                            let totalHp = matchedHero.baseHp + window.hpBuffAmt; let hpText = window.hpBuffAmt > 0 ? `${totalHp} (${matchedHero.baseHp}+${window.hpBuffAmt})` : `${totalHp}`;
                            ...
                            dDesc.innerText += starText;
                        }
                        
                        if(videoBtn && videoCont) {
                            if(stars >= 5) { 
                                videoBtn.style.display = 'inline-block'; videoCont.style.display = 'none';
                                ...
                                videoBtn.style.display = 'none'; videoCont.style.display = 'none'; 
                            }
                        }
                    }
                } 
    ...
    let wallGrid = document.getElementById('display-wall-grid'); 
    if(wallGrid) {
        wallGrid.innerHTML = "";
        window.gachaData.wall.forEach((slotId, index) => {
            ...
            if(slotId !== null && enabledStickers.includes(slotId)) {
                ...
                if (slotId !== null && !enabledStickers.includes(slotId)) window.gachaData.wall[index] = null; 
            }
